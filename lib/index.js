...
async function generateThumbnails(file, sizes, optimizeOptions) {
  return map(sizes, async ({ name, resizeOptions }) => {
    ...
    const path = `images/${name}/${file.hash}${file.ext}`
    ...
    return { buffer, path, mime: file.mime }
  })
}
...
function getS3BaseUrl({ cdn, params, region }) {
  ...
  return format('https://{0}.s3.{1}.digitaloceanspaces.com', [
    params.Bucket,
    region,
  ])
}

function getS3Instance({ endpoint, accessKeyId, secretAccessKey, region }) {
